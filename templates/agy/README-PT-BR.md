...
uma agent home persistente, autentique o Antigravity uma vez, e depois escolha se
...
- `Dockerfile`: Antigravity CLI mais ferramentas comuns de desenvolvimento Linux.
...
Autentique o Antigravity com uma destas opções:

- `GEMINI_API_KEY` no `.env`;
- OAuth do Google na TUI do Antigravity;
...
Este template não fornece um serviço daemon do Antigravity.

O Antigravity CLI roda como TUI interativa ou como comando one-shot. Se uma versão
futura do Antigravity CLI adicionar um modo daemon/server estável, adicione isso como
...
estado do Antigravity vindo da home persistente, copie só o estado que você aceita
expor para esse run:
...
Antigravity subir um app dentro de um run, publique só a porta necessária naquele
comando:
...
Monte a pasta do projeto que o Antigravity deve editar, e monte apenas os dados do
Antigravity que você aceita expor para aquele run.
